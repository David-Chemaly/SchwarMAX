...
# ═══════════════════════════════════════════════════════════════════════
#  Trajectory-storing integrator (for diagnostics / post-processing)
# ═══════════════════════════════════════════════════════════════════════

@partial(jax.jit, static_argnames=('acc_fn', 'pot_fn', 'N_max'))
def integrate_adaptive_trajectory(
    w0, acc_fn, pot_fn, N_max, T_total,
    dt_init=0.010, Omega=0.0,
    atol=1e-8, rtol=1e-6,
    dt_min=1e-5, dt_max=0.1,
):
    """Adaptive BS2(3) integrator that returns the full 6D trajectory.

    Same Bogacki-Shampine RK2(3) stepper as integrate_adaptive_barred,
    but without any binning.  Returns the raw phase-space trajectory for
    post-processing (velocity maps, orbit classification, etc.).

    Parameters
    ----------
    w0 : (6,)  Initial state [x, y, z, vx, vy, vz] in the bar frame.
    acc_fn, pot_fn : callables  (same as integrate_adaptive_barred)
    N_max : int   Maximum scan iterations (static).
    T_total : float   Target integration time.
    Others : adaptive step-size parameters.

    Returns
    -------
    y_traj : (N_max, 6)  Phase-space state at each scan iteration.
    dt_traj : (N_max,)   Time-step used (0 for rejected steps).
    t_traj : (N_max,)    Cumulative time at each iteration.
    valid : scalar        1.0 if orbit passes energy check, else 0.0.
    """
    k1_init = _deriv_barred(w0, acc_fn, Omega)
    EJ_0 = _compute_EJ(w0, pot_fn, Omega)
    ...
    def scan_body(carry, _):
        ...
        dt_out = jnp.where(accept, dt, 0.0)
        return (t_new, y_out, dt_new, k1_new), (y_out, dt_out, t_new)

    init_carry = (0.0, w0, dt_init, k1_init)
    (t_final, y_final, _, _), (y_traj, dt_traj, t_traj) = jax.lax.scan(
        scan_body, init_carry, xs=None, length=N_max
    )
    ...
    return y_traj, dt_traj, t_traj, valid
...
def integrate_adaptive_barred_withtraj(
    ...
    return Rzphi_bin_counts, surface_density, h1, h2, h3, h4, valid, n_accepted, T_integrated, o_traj, t_traj
...
def integrate_adaptive_withtraj_batch(w0, acc_fn, pot_fn, N_max, T_total,
    ...
    Rzphi_bin_counts, surface_density, h1, h2, h3, h4, valid, _, _, y_traj, t_traj = _integrate_adaptive_withtraj_vmap(
        w0, acc_fn, pot_fn, N_max, T_total,
        dt_init, Omega, atol, rtol, dt_min, dt_max,
        num_Vbin, bin_mapping, num_per_bin,
        Rzphi_minmax, XY_minmax,
    ...
    return Rzphi_bin_counts_out, surface_density_out, h1_out, h2_out, h3_out, h4_out, valid.sum(), y_traj, t_traj
