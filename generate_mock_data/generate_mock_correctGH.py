...
parser.add_argument('--alpha', type=float, default=float(os.environ.get('ALPHA', 40)),
...
parser.add_argument('--gamma', type=float, default=float(os.environ.get('GAMMA', 140)),
...
from ghMoments_new import gauss, H0, H1, H2, H3, H4, H5, H6, h_to_V_sigma
from utils import *
...
fig_name = data_folder + f'/plots/mock_Nbody_bar_XY_withRot_Nbins600_beta{int(beta)}_gamma{int(gamma)}_D{int(D/1000)}_gal2_try.png'
filename = path + f'/mock_data/mock_Nbody_bar_XY_withRot_Nbins600_beta{int(beta)}_gamma{int(gamma)}_D{int(D/1000)}_gal2_try.pkl'
...
XY_Vbin_index_unique = np.unique(XY_Vbin_index)
v0_new = []
s_new = []
for i in tqdm(range (0, len(v0))):
    mask = (XY_Vbin_index == XY_Vbin_index_unique[i])
    w = mass_data[mask]
    x = vy[mask]
    ...
    def optfnc(param):
        ...
        v, s = param[0], param[1]
        y, gau_val = gauss(x, v, s)
        H0_val = H0(y)
        H1_val = H1(y)
        H2_val = H2(y)
        h0 = np.sum(gau_val * H0_val * w)
        h1 = np.sum(gau_val * H1_val * w) / h0
        h2 = np.sum(gau_val * H2_val * w) / h0

        return h1**2 + h2**2

    _m, _s = sp.optimize.minimize(optfnc, (v0[i], s[i]), method='Nelder-Mead').x
...
_y, _gau = gauss(vy, v0_cell, s_cell)
_H0, _H1, _H2, _H3, _H4 = H0(_y), H1(_y), H2(_y), H3(_y), H4(_y)

h0 = jax.ops.segment_sum(_gau * mass_data * _H0, XY_Vbin_index, num_segments=np.amax(_bin_index_remap)+1)
h1 = jax.ops.segment_sum(_gau * mass_data * _H1, XY_Vbin_index, num_segments=np.amax(_bin_index_remap)+1)
h2 = jax.ops.segment_sum(_gau * mass_data * _H2, XY_Vbin_index, num_segments=np.amax(_bin_index_remap)+1)
h3 = jax.ops.segment_sum(_gau * mass_data * _H3, XY_Vbin_index, num_segments=np.amax(_bin_index_remap)+1)
h4 = jax.ops.segment_sum(_gau * mass_data * _H4, XY_Vbin_index, num_segments=np.amax(_bin_index_remap)+1)
...
cb2 = ax1[0,1].scatter(df_XY_merged['X_grid'], df_XY_merged['Y_grid'], c=df_XY_merged['V_mean'], 
                s = 20, cmap='turbo', marker = 's', vmin = -200, vmax=200)
...
cb3 = ax1[0,2].scatter(df_XY_merged['X_grid'], df_XY_merged['Y_grid'], c=df_XY_merged['V_sigma'], 
                s = 20, cmap='turbo', marker = 's', vmin = 0, vmax=200)
...
cb4 = ax1[1,0].scatter(df_XY_merged['X_grid'], df_XY_merged['Y_grid'], c=df_XY_merged['h1'],
                s = 20, cmap='turbo', marker = 's', vmin = -0.0001, vmax=0.0001)
...
cb6 = ax1[1,2].scatter(df_XY_merged['X_grid'], df_XY_merged['Y_grid'], c=df_XY_merged['h3'],
                s = 20, cmap='turbo', marker = 's', vmin = -0.2, vmax=0.2)
