...
parser.add_argument('--alpha', type=float, default=float(os.environ.get('ALPHA', 40)),
                    help='Euler angle alpha in degrees (default: 40)')
...
parser.add_argument('--gamma', type=float, default=float(os.environ.get('GAMMA', 140)),
                    help='Euler angle gamma in degrees (default: 140)')
...
fig_name = data_folder + f'/plots/mock_Nbody_bar_XY_withRot_Nbins600_beta{int(beta)}_gamma{int(gamma)}_D{int(D/1000)}_gal2_noh3h4.png'
filename = path + f'/mock_data/mock_Nbody_bar_XY_withRot_Nbins600_beta{int(beta)}_gamma{int(gamma)}_D{int(D/1000)}_gal2_noh3h4.pkl'
...
bin_dict = {
    ...
    'h1': np.array(h1),
    'h2': np.array(h2),
    'h3': np.zeros_like(h3), # set h3 to zero for simplicity, since the fitting of h3 is not very robust in low S/N bins
    'h4': np.zeros_like(h4),
    'v0': np.array(v0),
    's': np.array(s),
...
bin_dict['h3_err'] = np.ones_like(bin_dict['h3']) * 0.15
bin_dict['h4_err'] = np.ones_like(bin_dict['h4']) * 0.15
