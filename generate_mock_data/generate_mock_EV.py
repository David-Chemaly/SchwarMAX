...
parser.add_argument('--ev-ghmoments', type=bool, default=False, help="use EV's method for computing Gauss-Hermite moments")
parser.add_argument('--D', type=float, default=float(os.environ.get('D', 50000)),
                    help='Distance to galaxy in kpc (default: 50000)')
parser.add_argument('--alpha', type=float, default=float(os.environ.get('ALPHA', 40)),  # EV: changed from 10 to 40 as in the paper
...
parser.add_argument('--gamma', type=float, default=float(os.environ.get('GAMMA', 140)),  # EV: changed from 170 to 140
...
# EV: disable unnecessary imports
...
# EV: copied from integrants_with_binning
@jax.jit
...
field_of_view_Y = 40 # arcsec, field of view in Y direction  # EV: changed to 40 as in the paper
...
print(f"Rotation angles (alpha, beta, gamma): ({alpha}Â°, {beta}Â°, {gamma}Â°)")

target_sn = 40
print(f"Target S/N per Voronoi bin: {target_sn}")



fig_name = 'figure.png' #data_folder + f'/plots/mock_Nbody_bar_XY_withRot_Nbins600_beta{int(beta)}_gamma{int(gamma)}_D{int(D/1000)}_gal2.png'
filename = 'pickle.pkl' #path + f'/mock_data/mock_Nbody_bar_XY_withRot_Nbins600_beta{int(beta)}_gamma{int(gamma)}_D{int(D/1000)}_gal2.pkl'
...
w0_data, mass_data = agama.readSnapshot('t_t0_7')#snap_t0_3
...
offset_origin = w0_data_init - w0_data[0]
...
print('shifted snapshot by %s and rotated by %.10g' % (offset_origin, bar_angle0))
...
# Add PSF smearing 
x = np.random.normal(x, PSF_kpc, size=x.shape)
z = np.random.normal(z, PSF_kpc, size=z.shape)
XY_stars = np.stack([x, z], axis=-1) # prefect edge-on view


X_min, X_max = -field_of_view_X_kpc/2, field_of_view_X_kpc/2
Y_min, Y_max = -field_of_view_Y_kpc/2, field_of_view_Y_kpc/2
nX, nY = field_of_view_X, field_of_view_Y
...
# EV: the above code does not produce correct Gauss-Hermite moments; below is the correct method

def computeGHmoments(x, w, ghorder):
    def getHermitePolynomials(y, order):
        # y = (x - m) / s   is the scaled variable
        hpp = 1.0 + y*0
        hp  = 2**0.5 * y
        result = [hpp, hp]
        for m in range(2, order+1):
            hm = (2**0.5 * y * hp - (m-1)**0.5 * hpp) / m**0.5
            result.append(hm)
            hpp = hp
            hp  = hm
        return numpy.array(result)

    def getGHcoefs(m, s, order):
        y = (x-m) / s
        gaussian = (2*numpy.pi)**-0.5 / s * numpy.exp(-0.5 * y**2)
        hpoly = getHermitePolynomials(y, order)
        return numpy.sum(gaussian * hpoly * w, axis=1)

    # initial guess for m (center) and s (width of the base Gaussian) - classical 1st and 2nd moments
    m = numpy.sum(w*x)    / numpy.sum(w)
    s =(numpy.sum(w*x**2) / numpy.sum(w) - m**2)**0.5
    #print('initial guess: m=%g, s=%g' % (m,s))
    # tune m and s such that h1=h2=0;
    # note: the optimization could be performed more efficiently using analytic gradient of optfnc w.r.t. its arguments
    def optfnc(param):
        h1, h2 = getGHcoefs(param[0], param[1], 2)[1:3]
        return h1**2 + h2**2
    m, s = scipy.optimize.minimize(optfnc, (m,s), method='Nelder-Mead').x
    #print('optimized: m=%g, s=%g' % (m,s))
    # then compute the remaining Gauss-Hermite moments using the optimized m and s for the base Gaussian
    coefs = getGHcoefs(m, s, ghorder)
    # return center, width, h1, h2, h3, h4...; h1 and h2 are (nearly) zero by construction
    return numpy.hstack([m, s, coefs[1:] / coefs[0]])

if args.ev_ghmoments:  # EV: corrected method for GH moments
    import numpy, scipy.optimize
    nbins = numpy.amax(_bin_index_remap)+1
    result = numpy.zeros((nbins, 6))
    # I guess this could be somehow vectorised using jax, but lazy to figure it out
    for bin_index in tqdm(range(nbins)):
        filt = (XY_Vbin_index == bin_index)
        result[bin_index] = computeGHmoments(vy[filt], mass_data[filt], 4)

    V_mean, V_sigma, h1, h2, h3, h4 = result.T
...
# EV: change colormaps and climits
...
cb2 = ax1[0,1].scatter(df_XY_merged['X_grid'], df_XY_merged['Y_grid'], c=df_XY_merged['V_mean'], 
                s = 20, cmap='breeze', marker = 's', vmin = -200, vmax=200)
...
cb3 = ax1[0,2].scatter(df_XY_merged['X_grid'], df_XY_merged['Y_grid'], c=df_XY_merged['V_sigma'], 
                s = 20, cmap='breeze', marker = 's', vmin = 0, vmax = 200)
...
cb4 = ax1[1,0].scatter(df_XY_merged['X_grid'], df_XY_merged['Y_grid'], c=df_XY_merged['h1'],
                s = 20, cmap='breeze', marker = 's', vmin = -1e-4, vmax = 1e-4)
...
cb6 = ax1[1,2].scatter(df_XY_merged['X_grid'], df_XY_merged['Y_grid'], c=df_XY_merged['h3'],
                s = 20, cmap='breeze', marker = 's', vmin = -0.2, vmax = 0.2)
