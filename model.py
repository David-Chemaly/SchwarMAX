...
# from CylindricalSpline import evaluate_phi, get_acc                                                                                         
                                                        
# path = '/Users/hanyuan/Desktop/PhD_projects/SchwarMAX_data/Bar_model_TG21/dict_phi_stellar_t_t0_4.pkl'
# with open(path, 'rb') as f:
#     d = pickle.load(f)
# dict_phi = {k: jnp.array(v) for k, v in d.items() if k != '_metadata'}

# # Evaluate potential — returns (kpc/Gyr)^2
# phi = evaluate_phi(x, y, z, dict_phi)

# # Evaluate acceleration — returns (kpc/Gyr^2)
# acc = get_acc(x, y, z, dict_phi)  # single point
# acc = jax.vmap(get_acc, in_axes=(0,0,0,None))(x, y, z, dict_phi)  # vectorized
...
def get_jeans_moments_fixed(x_star, y_star, z_star, dict_phi_baryon, params_halo, anisotropy_b=1.0):
    ...
    Parameters
    ----------
    dict_phi_baryon : dict
        Pre-computed CylindricalSpline potential dict for the baryonic component.
    params_halo : dict
    ...
    nu_val = nu(x_star, y_star, z_star)
    ...
    # --- Step 3: v_phi from radial Jeans equation ---
    def vertical_pressure(r_in):
        def integrand_r(z_prime):
    ...
    term1 = sigma_R2
    term2 = (R_star / nu_val) * d_nu_sigR2_dR
    term3 = R_star * dPhi_dR(x_star, y_star, z_star, dict_phi_baryon, params_halo)
...
@jax.jit
def compute_model_and_logl_bootstrap_amp(
    weights_all,
    A_Rzphi, A_xy, A_h1, A_h2, A_h3, A_h4,
    y_Rzphi,
    y_xy_boot, y_h1_boot, y_h2_boot, y_h3_boot, y_h4_boot,
...
def model_fixed_potential(params_halo_pot, dict_phi_baryon, Omega_bar, light_to_mass_ratio, dict_data, num_Vbin):
    """
    Schwarzschild model with a fixed baryonic potential from a pre-computed
    CylindricalSpline dict (e.g. from get_phi_m_from_agama).

    Free parameters: NFW halo (logM, Rs), light_to_mass_ratio, Omega_bar.
    Fixed: baryonic potential (dict_phi_baryon), viewing angles (alpha=30, beta=20, gamma=140).

    Parameters
    ----------
    params_halo_pot : dict
        NFW halo parameters: logM, Rs, a, b, c, x/y/z_origin, dirx/y/z.
    dict_phi_baryon : dict
        Pre-loaded CylindricalSpline potential dict for the baryonic component.
    Omega_bar : float
        Bar pattern speed in rad/Gyr.
    light_to_mass_ratio : float
        Baryonic light-to-mass ratio.
    dict_data : dict
        Observational data (w0, v0, s, bin_mapping, density data, kinematics, etc.).
    num_Vbin : int
        Number of Voronoi bins.

    Returns
    -------
    Same as model(): density_set, V_model, sigma_model, h1-h4 sets, weights.
    """
    ...
    get_jeans_moments_fixed_vmap = jax.vmap(get_jeans_moments_fixed, in_axes=(0,0,0,None,None,None))

    def get_w0_new(w0, key1, key2, key3, n_particles):
        jeans_moments = get_jeans_moments_fixed_vmap(w0[:,0], w0[:,1], w0[:,2], dict_phi_baryon, params_halo_pot, 1.)
        v_rot, sig_R, sig_z, sig_phi = jeans_moments
        g1, g2, g3 = jax.random.normal(key1, (n_particles,)), jax.random.normal(key2, (n_particles,)), jax.random.normal(key3, (n_particles,))
        vR = g1 * sig_R
        vz = g2 * sig_z
    ...
    _Vc = jax.vmap(get_rotation_curve, in_axes=(0, None, None, 0))(
        _R,
        potential_func,
        (dict_phi_baryon, params_halo_pot),
        _z
    )
    ...
    def acc_fn(x, y, z):
        a_halo = NFW_acceleration(x, y, z,  params_halo_pot)
        a_disk = get_acc(x, y, z, dict_phi_baryon)
        return a_halo + a_disk
...
def model_fixed_potential_diagnostic(params_halo_pot, dict_phi_baryon,
                                     ...
                                     batch_size=500):
    """
    Run the fixed-potential Schwarzschild model and return orbit weights + full 6D trajectories.

    First calls model_fixed_potential_bootstrap to get NNLS orbit weights, then re-integrates
    ...
    Parameters
    ----------
    params_halo_pot, dict_phi_baryon, Omega_bar, light_to_mass_ratio, dict_data, num_Vbin :
        Same as model_fixed_potential_bootstrap.
    batch_size : int
    ...
    dict with keys:
        ...
        y_traj : (n_orbits, n_realizations, N_max, 6)
            Full 6D phase-space trajectory for each orbit in the bar frame.
        t_traj : (n_orbits, n_realizations, N_max)
            Cumulative time at each iteration.
        rotation_matrix : (3, 3)
        Omega_bar, logl_marg, logl_all, weights_all_boot,
        V_all, sigma_all, density_all, h1_all..h4_all
    """
    alpha, beta, gamma = dict_data['alpha'], dict_data['beta'], dict_data['gamma']
    rotation_matrix = makeRotationMatrix(alpha, beta, gamma)
    ...
    #=========================================== Integrate orbits (with trajectories) ===================================
    @jax.jit
    def acc_fn(x, y, z):
        a_halo = NFW_acceleration(x, y, z, params_halo_pot)
        a_disk = get_acc(x, y, z, dict_phi_baryon)
    ...
    return {
        'weights': np.array(weights_all[0]).astype(np.float32),
        'y_traj': y_traj.astype(np.float32),
        't_traj': t_traj.astype(np.float32),
...
def model_fixed_potential_bootstrap_amp(params_halo_pot, dict_phi_baryon,
    ...
    logl_marg, density_all, h1_all, h2_all, h3_all, h4_all, V_all, sigma_all, logl_all, m_eff = \
        compute_model_and_logl_bootstrap_amp(
            weights_all,
            A_Rzphi, A_xy, A_h1, A_h2, A_h3, A_h4,
            y_Rzphi,
            y_xy_boot, y_h1_boot, y_h2_boot, y_h3_boot, y_h4_boot,
            sig_Rzphi, sig_xy, sig_A1, sig_A2, sig_A3, sig_A4,
            v0, s, 
            sigma_amplifier, light_to_mass_ratio, mean_mass_per_orb
        )

    return weights_all, logl_marg, density_all, h1_all, h2_all, h3_all, h4_all, V_all, sigma_all, logl_all, m_eff


def model_fixed_potential_bootstrap_both(params_halo_pot, dict_phi_baryon, Omega_bar, light_to_mass_ratio, dict_data, num_Vbin):
    """
    Same as model_fixed_potential_bootstrap but computes m_eff from BOTH:
    ...
    -------
    dict with keys:
        Data bootstrap: weights_data, logl_marg_data, density_data, h1-h4_data,
                        V_data, sigma_data, logl_data, m_eff_data
        Model bootstrap: weights_model, logl_marg_model, density_model, h1-h4_model,
                         V_model, sigma_model, logl_model, m_eff_model, m_eff_lipka
        Best-fit: weights_best
    """
    ...
    # Reuse the same noise realization: noise = (y_boot_data - y_obs)
    ...
    # Lipka m_eff using the proper reference (l_fit, not bootfit[0])
...
def model_for_plotting(params_halo_pot, params_disk_rho, dict_data, num_Vbin,
    ...
    bin_mapping = dict_data['bin_mapping']
    Omega_bar = params_disk_rho['Omega_bar']
    light_to_mass_ratio = params_disk_rho['light_to_mass_ratio']
    alpha, beta, gamma = params_disk_rho['alpha'], params_disk_rho['beta'], params_disk_rho['gamma']
    rotation_matrix = makeRotationMatrix(alpha, beta, gamma)
    ...
    #===================================== Also compute unity-weight results for comparison =========================================
    ...
    h1_model_unity = jnp.clip(h1_model_unity, -clip_val, clip_val)
    h2_model_unity = jnp.clip(h2_model_unity, -clip_val, clip_val)
    h3_model_unity = jnp.clip(h3_model_unity, -clip_val, clip_val)
    h4_model_unity = jnp.clip(h4_model_unity, -clip_val, clip_val)
...
def model_fixed_potential_plotting(params_halo_pot, dict_phi_baryon, 
                                    Omega_bar, light_to_mass_ratio, 
                                    dict_data, num_Vbin,
                                    Rzphi_n_tot = 360, Rzphi_n_grid = jnp.array([10,6,6]), Rzphi_lim_grid = jnp.array([[0,10.],[-3,3],[-jnp.pi, jnp.pi]]),
                                    xy_lim_grid = jnp.array([[-10.,10.],[-3.,3.]]),
    ...
    y_Rzphi = dict_data['mass_in_grid']
    ...
    sig_Rzphi = 0.02 * y_Rzphi + 1e-10 #0.02 * y_Rzphi + 1e-10
    sig_xy = (dict_data['XY_density_data_err'] + EPSILON) / light_to_mass_ratio
    sig_A1 = dict_data['h1_data_err'] + EPSILON
    sig_A2 = dict_data['h2_data_err'] + EPSILON
    sig_A3 = dict_data['h3_data_err'] + EPSILON
    ...
    V_model, sigma_model = h_to_V_sigma(h1_model, h2_model, v0, s)
    density_2DXY = density_2DXY * mean_mass_per_orb * light_to_mass_ratio  # convert back to luminosity units for density
    y_xy = y_xy * mean_mass_per_orb * light_to_mass_ratio  # convert back to luminosity units for density
    sig_xy = sig_xy * mean_mass_per_orb * light_to_mass_ratio  # convert back to luminosity units for density
    ...
    V_model_unity, sigma_model_unity = h_to_V_sigma(h1_model_unity, h2_model_unity, v0, s)
    density_2DXY_unity = density_2DXY_unity * mean_mass_per_orb * light_to_mass_ratio  # convert back to luminosity units for density
