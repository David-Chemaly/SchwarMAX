...
from model_bar import model, model_marg, model_bootstrap, model_single, model_bootstrap_psf, projection, density_func, model_test_convergence, model_jackknife_chi2
...
@partial(jax.jit, static_argnames=('num_Vbin', 'Rzphi_n_tot'))
def logl_angular_input_single(params, dict_data, num_Vbin, Rzphi_n_tot):
    """
    Same as logl_angular_input but uses model_bootstrap() which marginalizes
    logL over observational noise via bootstrap resampling:
        - 100 perturbed observation vectors y_i = y + N(0, sig)
    ...
    def true_func():
        return -jnp.inf
    def false_func():
        weights_all, logl, density_all, h1_all, h2_all, h3_all, h4_all, V_all, sigma_all = \
            model_single(params_halo_pot, params_baryon_rho, dict_data, num_Vbin,
                        Rzphi_n_tot, Rzphi_n_grid, Rzphi_lim_grid=jnp.array([[Rmin, Rmax],[zmin, zmax],[phimin, phimax]]),
                        xy_lim_grid=xy_lim_grid,
                        xy_n_grid=xy_n_grid)
