...
@jax.jit
def _outside_outer_domain(Rscaled, zscaled, dict_phi_m):
    """Mask points outside the outer interpolation box."""
    return (Rscaled > dict_phi_m['Rgrid'][-1]) | (zscaled > dict_phi_m['Zgrid'][-1])

@jax.jit
def _outer_monopole_phi_dphi_dR_dzabs(R, z_abs, dict_phi_m):
    """
    Outer fallback used outside the spline box:
        phi = -GM / r + C
    Returns phi, dphi/dR, dphi/d|z|.
    """
    outer_GM = dict_phi_m['outer_GM'][0]
    outer_C = dict_phi_m['outer_C'][0]
    r2 = R * R + z_abs * z_abs + 1e-12
    invr = 1.0 / jnp.sqrt(r2)
    phi = -outer_GM * invr + outer_C
    coef = outer_GM * invr * invr * invr
    dphi_dR = coef * R
    dphi_dzabs = coef * z_abs
    return phi, dphi_dR, dphi_dzabs
...
def get_phi_m(rho_fn, params, NR, NZ, Rmin, Rmax, Zmin, Zmax, Mmax, Nphi, N_int):
    ...
    # Outer fallback coefficients for points outside (Rmax, Zmax):
    # fit a monopole phi ~ -GM/r + C from boundary values of m=0 potential.
    phi0 = phi_real[0]
    RR = R[:, None]
    ZZ = Z_nonneg[None, :]
    r_boundary = jnp.sqrt(RR * RR + ZZ * ZZ + 1e-12)
    bmask = (jnp.arange(R.shape[0])[:, None] == (R.shape[0] - 1)) | \
            (jnp.arange(Z_nonneg.shape[0])[None, :] == (Z_nonneg.shape[0] - 1))
    gm_raw = -phi0 * r_boundary
    pos_mask = bmask & (gm_raw > 0.0)
    pos_count = jnp.sum(pos_mask.astype(phi0.dtype))
    gm_mean = jnp.sum(jnp.where(pos_mask, gm_raw, 0.0)) / jnp.maximum(pos_count, 1.0)
    gm_corner = jnp.maximum(gm_raw[-1, -1], 0.0)
    outer_GM = jnp.where(pos_count > 0.0, gm_mean, gm_corner)
    bcount = jnp.maximum(jnp.sum(bmask.astype(phi0.dtype)), 1.0)
    outer_C = jnp.sum(jnp.where(bmask, phi0 + outer_GM / r_boundary, 0.0)) / bcount
    ...
    Phi_m_grid = {
        ...
        'Mx_img': phi_Mx_img,
        'My_img': phi_My_img,
        'outer_GM': jnp.array([outer_GM]),
        'outer_C': jnp.array([outer_C]),
    }
...
def evaluate_phi(x, y, z, dict_phi_m):
    ...
    phi_in = jnp.sum(phi_grid, axis=0)
    R = jnp.sqrt(x*x + y*y)
    z_abs = jnp.abs(z)
    R0 = dict_phi_m['R0'][0]
    _R, _z = _Rz_rescale_forward(R, z_abs, R0)
    outside = _outside_outer_domain(_R, _z, dict_phi_m)
    phi_out, _, _ = _outer_monopole_phi_dphi_dR_dzabs(R, z_abs, dict_phi_m)
    return jnp.where(outside, phi_out, phi_in)
...
def evaluate_phi_axisymmetric(x, y, z, dict_phi_m):
    ...
    outside = _outside_outer_domain(_R, _z, dict_phi_m)
    phi_out, _, _ = _outer_monopole_phi_dphi_dR_dzabs(R, z_abs, dict_phi_m)
    return jnp.where(outside, phi_out, val_final)
...
def _evaluate_phi_and_scaled_derivs(x, y, z, dict_phi_m):
    ...
    outside = _outside_outer_domain(_R, _z, dict_phi_m)
    phi_out, dphi_dR_out, dphi_dzabs_out = _outer_monopole_phi_dphi_dR_dzabs(R, z_abs, dict_phi_m)
    dRsc_dR = 1.0 / jnp.sqrt(R * R + R0 * R0)
    dzsc_dzabs = 1.0 / jnp.sqrt(z_abs * z_abs + R0 * R0)
    dphi_dRsc_out = dphi_dR_out / jnp.maximum(dRsc_dR, 1e-12)
    dphi_dzsc_out = dphi_dzabs_out / jnp.maximum(dzsc_dzabs, 1e-12)
    phi = jnp.where(outside, phi_out, phi)
    dphi_dRsc = jnp.where(outside, dphi_dRsc_out, dphi_dRsc)
    dphi_dzsc = jnp.where(outside, dphi_dzsc_out, dphi_dzsc)
    dphi_dph = jnp.where(outside, 0.0, dphi_dph)
